<div class="no-grid">
	<h1><?php echo $app->t($contr->pageTitle) ?></h1>

	<form class="hide" id="formConfirm" method="post" action="<?php echo $_SERVER['REQUEST_URI'] ?>">
		<fieldset>
			<p class="message notice">
				<?php echo $app->t($view->notice) ?>

				<br/><br/>

				<input type="hidden" name="get-data" value="<?php echo $view->getData ?>"/>

				<input type="hidden" name="auth-token" value="<?php echo $app->authToken ?>"/>

				<input type="submit" name="confirm" id="confirm" value="<?php echo $app->t('Yes, proceed') ?>"/>

				<a href="javascript: void(0);" onclick="history.go(-1);"><?php echo $app->t('No, go back') ?></a>
			</p>
		</fieldset>
	</form>
</div>
