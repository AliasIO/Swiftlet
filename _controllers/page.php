...
if ( isset($view->route['parts'][1]) )
{
	...
	$app->db->sql('
		...
		WHERE
			n.`id`        = "' . ( int ) $view->route['parts'][1] . '" AND
			n.`type`      = "page"                                     AND
			p.`published` = 1                                          AND
...
if ( !isset($view->nodeId) )
{
	...
	$view->pageTitle = $view->t('Page not found');
	$view->error     = $view->t('The page you are looking for does not exist.');
}
